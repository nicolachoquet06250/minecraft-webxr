[⬅️ Precedent](./blocks.md) | [Sommaire](./README.md) | [Suivant ➡️](./crafts.md)

---

# Items disponibles

Cette page liste les items actuellement disponibles dans le jeu.

## Fonctionnement general

Tous les blocs definis sauf `Air` ont un item associe. Les items de blocs ont une taille maximale de stack de 64. Les outils peuvent avoir une limite differente.

## Items de blocs

Tous les blocs listes dans [Blocs disponibles et ou les trouver](./blocks.md) existent aussi comme items, sauf `Air`.

## Items avec visuel dedie

| Visuel | Item | Nom francais | Details |
|--------|------|--------------|---------|
| <img src="../items/visuels/pickaxe-grass-dirt.png" alt="Dirt Grass Pickaxe" width="40" height="40" /> | Dirt Grass Pickaxe | pioche en terre et herbe | Stack max 1. |

## Categories

- Terrain
- Liquides neige et glace
- Minerais
- Bois feuilles et planches
- Plantes
- Decoration et utilitaires
- Laines
- Outils

---

[⬅️ Precedent](./blocks.md) | [Sommaire](./README.md) | [Suivant ➡️](./crafts.md)
